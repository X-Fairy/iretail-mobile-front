		...
		<view slot="body" @tap.stop="longClose()">
			<view class="top">
				...
				<view class="header">
					...
					<view class="right" @click="showDrawer('showRight')">
					...
					<view class="order-item">
						<view class="right">
							<view class="right-status">
								...
								<view class="time">创建于:{{item.createTime}}</view>
							</view>
							<view @tap="toDetail(item)">
								...
								<view class="bottom">
									<view class="dateInfo">
										...
										<text class="icon iconfont Graph-PieChart time-icon" v-if="item.orderTypeCode != '1003'"></text>
										<text v-if="item.orderTypeCode != '1003'">交期：{{item.deliveryDate}}</text>
...
<script>
	...
	import longPop from "@/components/long-pop/long-pop.vue";
	...
	export default {
		...
		onShow() {
			this.getPersonList()
			// #ifndef H5
			this.$utils.getAnaInfo("query_order");
			// #endif
			// 详情页删除操作跳回
			if (uni.getStorageSync("removeOrderId")) {
				this.orderList.forEach((item, index) => {
					if (item.orderId == uni.getStorageSync("removeOrderId")) {
						this.orderList.splice(index, 1);
					}
				});
				uni.removeStorageSync("removeOrderId");
			}
						...
						if (res.code == "01") {
							if (res.data.length > 0) {
								...
								this.orderList.forEach(ele => {
									...
									ele.createTime = utils.changeDay(new Date(ele.createTime));
									ele.showPop = false;
								});
					...
					if (e.orderTypeCode == '1003') {
						uni.navigateTo({
							url: "../../addOrder/addZtOrder?orderId=" +
								...
								"&statusCode=" +
								e.statusCode +
								"&type=orderDetail&orderTypeCode=" + e.orderTypeCode
						});
					} else {
						uni.navigateTo({
