...
<template>
	<view class="datatime">
		<picker mode="multiSelector" :range="range" range-key="text" @change="change" @columnchange="columnchange" :value="value" :disabled="isDisabled">
			<view class="content" :class="(placeholder && !dateStr) || isDisabled ? 'placeholder' : ''">
				<text v-if="dateStr">{{ dateStr | formatDate }}</text>
...
export default {
	...
	watch: {
		...
		defaultValue(value) {
			if (value) {
				let dt = null;
				if (Number(value)) {
					dt = utils.formatDate(new Date(Number(value)))
				} else {
					dt = utils.formatDate(value)
				}
				this.defaultValue = dt.f1
				this.dateStr = dt.f1
			}
			// 设置默认值
	...
	created() {
		...
		// 验证是否是有效的开始和结束日期
		if(!this.start || !utils.isString(this.start)) {
			console.log('开始日期需为String类型，格式为 "YYYY-MM-DD hh:mm"');
			start = '1970-1-1 00:00';
		}
		if(!this.end || !utils.isString(this.end)) {
